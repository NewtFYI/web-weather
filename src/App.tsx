...
import type { TempUnit, WeatherCity, WeatherForecastData, WeatherLocation } from "./types/weather.ts";
...
function App() {
	...
	return (
		<div className="relative mx-auto flex min-h-screen max-w-270 flex-col pt-10 pb-10">
			<LocationHeader location={locationData} current={weatherData.current} onCitySelected={onSelectCity} />
			<Hero day={weatherData.current} unit={selectedUnit} onUnitChange={(unit) => setSelectedUnit(unit)} />
			<ul>
				{weatherData.forecast.hours.map((hour) => {
					return (
						<div key={hour.epoch}>
							<div>{hour.epoch}</div>
						</div>
